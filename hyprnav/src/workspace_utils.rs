...
pub fn build_workspace_descriptors(monitors_json: &[u8], workspaces_json: &[u8], clients_json: &[u8]) -> Vec<WorkspaceDescriptor> {
    let monitors = serde_json::from_slice::<Vec<MonitorInfo>>(monitors_json).unwrap_or_default();
    let workspaces = serde_json::from_slice::<Vec<WorkspaceInfo>>(workspaces_json).unwrap_or_default();
    ...
    for client in clients.into_iter().filter(|client| client.mapped && client.workspace.id > 0) {
        let entry = client_info_by_workspace.entry(client.workspace.id).or_insert_with(|| WorkspaceClientInfo {
    ...
    let mut descriptors = workspaces
        ...
        .map(|workspace| {
            ...
            let app_class = client_info.map(|item| item.app_class.clone()).unwrap_or_default();
            let subtitle = client_info
                .filter(|item| !item.title.is_empty())
                .map(|item| item.title.clone())
                .or_else(|| (!app_class.is_empty()).then_some(app_class.clone()))
                .or_else(|| (!workspace.last_window_title.is_empty()).then_some(workspace.last_window_title.clone()))
            ...
            WorkspaceDescriptor {
                ...
                subtitle,
                app_class,
                window_count: client_info.map(|item| item.window_count).unwrap_or(workspace.windows),
                focus_history_rank: client_info.map(|item| item.focus_history_rank).unwrap_or(i32::MAX),
...
pub fn sort_workspaces_for_switcher(workspaces: &mut [WorkspaceDescriptor], mru_workspace_ids: &[i32]) {
    workspaces.sort_by(|left, right| {
        left.active
            .cmp(&right.active)
            .reverse()
            .then_with(|| mru_rank(mru_workspace_ids, left.id).cmp(&mru_rank(mru_workspace_ids, right.id)))
            .then_with(|| left.focus_history_rank.cmp(&right.focus_history_rank))
            .then_with(|| left.id.cmp(&right.id))
    });
}
