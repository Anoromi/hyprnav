...
CPreviewManager::CPreviewManager() {
    if (PHANDLE) {
        static auto* const* PPREVIEWHEIGHT = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprnav-plugin:preview_height")->getDataStaticPtr();
...
void CPreviewManager::createSocket() {
    ...
    if (m_serverFD < 0) {
        Log::logger->log(Log::ERR, "[hyprnav-plugin] failed to create preview socket");
        return;
    }
    ...
    if (socketString.size() >= sizeof(addr.sun_path)) {
        Log::logger->log(Log::ERR, "[hyprnav-plugin] preview socket path too long");
        close(m_serverFD);
        m_serverFD = -1;
        return;
    }
    ...
    if (bind(m_serverFD, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0) {
        Log::logger->log(Log::ERR, std::format("[hyprnav-plugin] failed to bind preview socket: {}", std::strerror(errno)));
        close(m_serverFD);
        m_serverFD = -1;
        return;
    }
    ...
    if (listen(m_serverFD, 8) < 0) {
        Log::logger->log(Log::ERR, std::format("[hyprnav-plugin] failed to listen on preview socket: {}", std::strerror(errno)));
        close(m_serverFD);
        m_serverFD = -1;
        return;
    }
...
void CPreviewManager::refreshRuntimePaths() {
    const auto runtimeDir = hyprnav_plugin::runtimeDirectory(std::getenv("XDG_RUNTIME_DIR"), previewInstanceSignature());
    const auto socketPath = hyprnav_plugin::socketPath(std::getenv("XDG_RUNTIME_DIR"), previewInstanceSignature());
...
void CPreviewManager::acceptClients() {
    while (true) {
        ...
        if (clientFD < 0) {
            ...
            Log::logger->log(Log::ERR, std::format("[hyprnav-plugin] failed to accept preview client: {}", std::strerror(errno)));
            break;
        }
        ...
        if (!sendLine(clientFD, hyprnav_plugin::formatHelloEvent(m_socketPath, m_previewHeight)))
    ...
    for (auto& [fd, client] : m_clients) {
        ...
        while (true) {
            ...
            if (client.readBuffer.size() > MAX_CLIENT_LINE) {
                sendLine(fd, hyprnav_plugin::formatErrorEvent("command too long"));
                disconnected.push_back(fd);
                break;
            }
...
void CPreviewManager::handleClientLine(int fd, const std::string& line) {
    std::string error;
    const auto  command = hyprnav_plugin::parseClientCommand(line, error);
    if (!command.has_value()) {
        sendLine(fd, hyprnav_plugin::formatErrorEvent(error));
        return;
    }

    switch (command->command) {
        case hyprnav_plugin::eClientCommand::HELLO: sendLine(fd, hyprnav_plugin::formatHelloEvent(m_socketPath, m_previewHeight)); break;
        case hyprnav_plugin::eClientCommand::WATCH: {
            queueRefresh(command->workspaceIDs);
            wakeTimer();
            break;
        }
        case hyprnav_plugin::eClientCommand::REFRESH:
            queueRefresh(command->workspaceIDs);
            wakeTimer();
            break;
        case hyprnav_plugin::eClientCommand::CLEAR: break;
        case hyprnav_plugin::eClientCommand::PING: sendLine(fd, "{\"event\":\"pong\"}\n"); break;
    }
}
...
void CPreviewManager::refreshPending() {
    ...
    for (const auto workspaceID : refreshIDs) {
        ...
        hyprnav_plugin::SPreviewSize size;
...
void CPreviewManager::broadcastPreviewEvent(int workspaceID, const std::filesystem::path& path, int width, int height, uint64_t generation) {
    const auto payload = hyprnav_plugin::formatPreviewEvent(workspaceID, path, width, height, generation);
    std::vector<int> disconnected;
...
bool CPreviewManager::renderWorkspacePreview(PHLMONITOR monitor, int workspaceID, std::filesystem::path& outPath, hyprnav_plugin::SPreviewSize& outSize) {
    if (!monitor)
        return false;

    outSize = hyprnav_plugin::computePreviewSize(monitor->m_pixelSize.x, monitor->m_pixelSize.y, m_previewHeight);
