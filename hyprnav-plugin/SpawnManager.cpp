...
void CSpawnManager::createSocket() {
    ...
    if (bind(m_serverFD, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0) {
        Log::logger->log(Log::ERR, std::format("[hyprnav-plugin] failed to bind spawn socket: {}", std::strerror(errno)));
        close(m_serverFD);
        m_serverFD = -1;
        return;
    }
    ...
    if (listen(m_serverFD, 8) < 0) {
        Log::logger->log(Log::ERR, std::format("[hyprnav-plugin] failed to listen on spawn socket: {}", std::strerror(errno)));
        close(m_serverFD);
        m_serverFD = -1;
        return;
    }
...
void CSpawnManager::refreshRuntimePaths() {
    const auto runtimeDir = hyprnav_plugin::runtimeDirectory(std::getenv("XDG_RUNTIME_DIR"), spawnInstanceSignature());
    const auto socketPath = hyprnav_plugin::spawnSocketPath(std::getenv("XDG_RUNTIME_DIR"), spawnInstanceSignature());
...
void CSpawnManager::acceptClients() {
    while (true) {
        ...
        if (clientFD < 0) {
            ...
            Log::logger->log(Log::ERR, std::format("[hyprnav-plugin] failed to accept spawn client: {}", std::strerror(errno)));
            break;
        }
